...
export class ApisService {
  ...
  public addOrdinance(uid, id, param): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this.adb.collection('ordinances').doc(uid).collection('all').doc(id).set(param).then((data) => {
        resolve(data);
      }, error => {
        reject(error);
      });
  ...
  public getOrdinances(uid: any): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this.db.collection('ordinances').doc(uid).collection('all').get().then((data) => {
        var users = data.docs.map(doc => {
          var item = doc.data();
          item.cid.get().then(function (doc) {
            item.cid = doc.data();
  ...
  public getOrdinanceWithId(uid: any, id): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this.db.collection('ordinances').doc(uid).collection('all').doc(id).get().then((data) => {
        console.log('**', data.data());
        var item = data.data();
        item.id = data.id;
        data.data().cid.get().then(function (doc) {
